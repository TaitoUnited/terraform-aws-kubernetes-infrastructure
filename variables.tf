...
variable "kubernetes_min_node_count" {
  type        = number
  default     = 1
  description = "Minimum amount of Kubernetes nodes."
}
...
variable "postgres_tiers" {
  ...
  description = "Tier for each PostgreSQL cluster. Provide multiple if you require multiple clusters. NOTE: Currently supports only one PostgreSQL cluster as Terraform does not support count for modules."
}
